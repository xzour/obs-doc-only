...
export class MrdocApiReq{
    ...
    async uploadAttachment(file: ArrayBuffer, fileName: string): Promise<any> {
      const mrdocUrl = processMrdocUrl(this.plugin.settings.mrdocUrl);
      const mrdocToken = this.plugin.settings.mrdocToken;
      const apiUrl = `${mrdocUrl}/api/upload_file/?token=${mrdocToken}`;
      ...
      try {
        ...
        // 使用 Obsidian 提供的 requestUrl
        const response = await requestUrl({
          url: apiUrl,
          method: "POST",
          body: formData as any, // 注意：FormData 类型需要断言为 any
        });
        ...
        try {
          ...
          return { status: false, data: "解析响应失败" };
        }
      } catch (error: any) {
        console.error("上传附件请求出错：", error);
